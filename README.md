...
- 可选 LLM 工具 `search_doujin_by_current_image`，后台查询完成后同样发送缩略图结果。
...
支持 Function Calling 的模型可以调用 `search_doujin_by_current_image`。工具仅从当前事件或引用消息中读取图片，并再次执行白名单与冷却校验；它不接受任意 URL 或本地路径。工具取得图片后会立即转为内存数据，再启动后台查询，因此不会在最长 300 秒的搜索期间占住 Agent 对话线程；结果会直接发送到原会话，并包含相似度不低于 80% 的缩略图。
